...
class TimetableGenerator {
  ...
  getReadyTimetable() {
    ...
    for (let i = 0; i < lessons.length; i += 1) {
      ...
      const subject = timetable.getSubjectById(bestLesson.getSubjectId()).getSubjectCode();
      const group = timetable.getGroupById(bestLesson.getGroupId()).getGroupName();
      const teacher = timetable.getTeacherById(bestLesson.getTeacherId()).getTeacherName();
      const t = timetable.getTimeslotById(bestLesson.getTimeslotId()).getTimeslot();
      // const value =
  ...
  _initTimeslots(timetable) {
    for (let i = 0; i < this._timeslots.length; i += 1) {
  ...
  _initTeachers(timetable) {
    for (let i = 0; i < this._teachers.length; i += 1) {
      const currentTeacher = this._teachers[i];
      const { username } = currentTeacher;
      const id = i + 1;
  ...
  _initSubjects(timetable) {
    for (let i = 0; i < this._subjects.length; i += 1) {
      ...
      timetable.addSubject(id, currentSubject.code, currentSubject.name, teacherIds);
