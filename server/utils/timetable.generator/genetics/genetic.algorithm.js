...
class GeneticAlgorithm {
  ...
  mutatePopulation(population, timetable) {
    ...
    for (let pi = 0; pi < population.getPopulationSize(); pi += 1) {
      const individual = population.getFittest(pi);
      const randomIndividual = new Individual(timetable);

      for (let gi = 0; gi < individual.getChromosomeLength(); gi += 1) {
  ...
  crossoverPopulation(population) {
    ...
    for (let pi = 0; pi < population.getPopulationSize(); pi += 1) {
      ...
      if (this._crossoverRate > Math.random() && pi >= this._elitismCount) {
        const offspring = new Individual(parent1.getChromosomeLength());
        const parent2 = this.selectParent(population);

        for (let gi = 0; gi < parent1.getChromosomeLength(); gi += 1) {
          if (Math.random() < 0.5) {
            offspring.setGene(gi, parent1.getGene(gi));
          } else {
            offspring.setGene(gi, parent2.getGene(gi));
          }
        }
